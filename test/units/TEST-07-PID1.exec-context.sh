...
if ! systemd-detect-virt -cq; then
    ...
    if ! systemctl --version | grep -qF -- "-BPF_FRAMEWORK"; then
        ...
        # Verify that the socket is correctly bound to the interface
        ...
        # Verify that a socket without BindNetworkInterface is not bound
        # to the interface
        systemd-run --wait --pipe \
            bash -xec 'ncat -l 127.0.0.1 9998 & sleep 0.5; ! ss -tlnp | grep -q "127.0.0.1%vrf-test:9998"'
